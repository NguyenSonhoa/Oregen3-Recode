...
public class PluginUtils {
    ...
    public Generator getChosenGenerator(final UUID uuid, World world) {
        ...
        if (plugin.hasDependency()) {
            ...
            final OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(p);
            for (final Generator chooser : plugin.getDataManager().getGenerators().values()) {
                if (plugin.getPermissionChecker().checkPerm(world.getName(), offlinePlayer, chooser.getPermission())
                        && chooser.getPriority() >= mc.getPriority()
                        && plugin.getHook().getIslandLevel(p, null) >= chooser.getLevel()
                        && checkRealmRequirement(offlinePlayer, chooser)) {
                    mc = chooser;
                }
            }
        }
    ...
    private Generator getMaterialChooser(final Location loc, Generator mc, final OfflinePlayer p) {
        ...
        for (final Generator chooser : plugin.getDataManager().getGenerators().values()) {
            ...
            if (plugin.getPermissionChecker().checkPerm(loc.getWorld().getName(), p, chooser.getPermission())
                    ...
                    && checkRealmRequirement(p, chooser)) {
                mc = chooser;
            }
        }
        return mc;
    ...
    private boolean checkRealmRequirement(OfflinePlayer p, Generator generator) {
        int required = generator.getRealmRequired();
        if (required <= 0) {
            return true;
        }

        if (!Bukkit.getPluginManager().isPluginEnabled("TuTienCore")) {
            return false;
        }

        try {
            int playerRealm = TuTien.getApi().getRealmId(p.getUniqueId());
            return playerRealm >= required;
        } catch (Exception e) {
            return false;
        }
    }
