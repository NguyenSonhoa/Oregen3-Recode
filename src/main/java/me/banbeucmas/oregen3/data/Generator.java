...
public class Generator {
    ...
    Generator(Oregen3 plugin, final String id) {
        ...
        if (path.isSet("drops")) {
            replaceDrops = path.getBoolean("drops.replace", false);
            ConfigurationSection dropSection = path.getConfigurationSection("drops.items");
            if (dropSection != null) {
                for (final String drop : dropSection.getKeys(false)) {
